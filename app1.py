...
with st.sidebar:
    ...
    if st.session_state.user:
        ...
        if archived:
            labels = [c["label"] for c in archived]
            selected = st.selectbox("Open previous chat", options=["-"] + labels, index=0)
            if selected != "-":
                chosen = next((c for c in archived if c["label"] == selected), None)
                if chosen:
                    try:
                        with open(chosen["path"], "r", encoding="utf-8") as f:
                            st.session_state.messages = json.load(f)
                        # Also set this as the current quick history file
                        save_chat_history(st.session_state.user, st.session_state.messages)
                        st.rerun()
                    except Exception:
