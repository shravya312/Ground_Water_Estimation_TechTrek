...
def login():
    ...
    with st.form("login_form"):
        ...
        with col2:
            if st.form_submit_button("Register (for new users)"):
...
def logout():
    ...
    save_chat_history_to_file(st.session_state.chat_history) # Clear persistent chat history on logout
    st.rerun()
...
if not st.session_state.logged_in:
    ...
    # Sidebar for options (only visible when logged in)
    with st.sidebar:
        ...
        # New Chat Button
        if st.button("✨ New Chat", help="Start a new chat and save the current one to history"):
        ...
        if st.session_state.chat_history:
            for chat_id, messages in st.session_state.chat_history.items():
                if messages:
                    # Display the first user message as the chat title
                    first_message_content = messages[0]['content']
                    if st.sidebar.button(f"Chat: {first_message_content[:30]}..."):
    ...
    # --- Main App Logic (only visible when logged in) ---
