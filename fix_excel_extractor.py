#!/usr/bin/env python3
"""
Fixed Excel Extractor - Properly extract STATE column from INGRIS data
"""
...
def extract_data_from_excel_fixed(file_path, year=None):
    ...
    try:
        ...
        # IMPORTANT: Use the actual STATE column from the Excel data, not the extracted state
        if 'state' in df.columns:
            ...
            df['state'] = state
            logger.info(f"No STATE column in {file_path}, using extracted state: {state}")
...
def process_ingris_datasets_fixed():
    """Process all INGRIS datasets with fixed state extraction."""
    print("🔧 Processing INGRIS Datasets with Fixed State Extraction")
    print("=" * 60)
    ...
    for dataset_name, year in year_datasets:
        ...
        for file_path in excel_files:
            data = extract_data_from_excel_fixed(file_path, year)
    ...
    if all_data:
        ...
        # Save the fixed CSV
        output_file = "ingris_rag_ready_fixed_states.csv"
        df.to_csv(output_file, index=False)
        print(f"\n✅ Fixed CSV saved: {output_file}")
...
if __name__ == "__main__":
    df = process_ingris_datasets_fixed()
    if df is not None:
        print(f"\n🎉 Fixed extraction complete! {len(df)} rows with proper state data.")
