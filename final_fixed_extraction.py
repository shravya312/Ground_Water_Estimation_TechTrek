#!/usr/bin/env python3
"""
Final Fixed Extraction - Check multiple rows for state data
"""
...
def process_ingris_datasets_final_fixed():
    """Process all INGRIS datasets with final state extraction fix."""
    print("🔧 Final Fixed INGRIS Processing - Extract State from Multiple Rows")
    print("=" * 70)
    ...
    for dataset_name, year in year_datasets:
        ...
        for file_path in excel_files:
            data = extract_data_from_excel_final_fixed(file_path, year)
    ...
    if all_data:
        ...
        # Save the final CSV
        output_file = "ingris_rag_ready_final_fixed.csv"
...
if __name__ == "__main__":
    df = process_ingris_datasets_final_fixed()
    if df is not None:
